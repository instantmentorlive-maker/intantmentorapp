...
class NetworkErrorHandler {
  ...
  static Result<T> _handleDioError<T>(DioException error) {
    ...
    switch (error.type) {
      ...
      case DioExceptionType.badCertificate:
        return Failure(
          NetworkError(
            message: 'SSL certificate error. Please check your connection security.',
            code: 'SSL_ERROR',
          ),
        );
  ...
  static Result<T> _handleResponseError<T>(DioException error) {
    ...
    switch (statusCode) {
      ...
      case 500:
        return Failure(NetworkError.serverError('Internal server error. Please try again later.'));
        
      case 502:
        return Failure(NetworkError.serverError('Bad gateway. The server is temporarily unavailable.'));
        
      case 503:
        return Failure(NetworkError.serverError('Service unavailable. Please try again later.'));
        
      case 504:
        return Failure(NetworkError.serverError('Gateway timeout. The server is taking too long to respond.'));
        
      default:
        return Failure(NetworkError.serverError('Server error ($statusCode): $errorMessage'));
    }
  }
  
  /// Extract validation errors from response
  static Map<String, List<String>> extractValidationErrors(dynamic responseData) {
  ...
  static bool isRecoverable(AppError error) {
    ...
    if (error is AppGeneralError && error.code == 'PERMISSION_DENIED') return false;
    return false;
  }
